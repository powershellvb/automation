...
#!/bin/bash
sshpass -p ****** scp -p /usr/local/custom/bin/EPIC_ID_Create/input_generic.txt  qualys@<IP_SHEPICTEMDH202>:/tmp
sshpass -p ***** ssh -tt qualys@<IP_SHEPICTEMDH202>  'ksh -s' </usr/local/custom/bin/EPIC_ID_Create/generic_user_nc.ksh
exit
