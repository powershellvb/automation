...
/****** Object:  DdlTrigger [trg_SGMS_CreateDBTrigger]    Script Date: 7/8/2016 2:59:13 PM ******/
SET ANSI_NULLS ON
GO
...
GO
 ...
 CREATE TRIGGER [trg_SGMS_CreateDBTrigger]
ON ALL SERVER
FOR CREATE_DATABASE
AS
...
SET @subjectText = 'Database Created on ' + @@SERVERNAME + ' by ' +  SUSER_SNAME() + '      '  +convert( varchar(40), GETDATE())
set @servername= @@SERVERNAME
set @username=  SUSER_SNAME()
SET @results = 'This Database  has been created  by  ' +  SUSER_SNAME()  +  ' on server'+ ' ' + @@SERVERNAME + '     '+ convert( varchar(40), GETDATE()) + ' .'+ '       ' + (SELECT EVENTDATA().value('(/EVENT_INSTANCE/TSQLCommand/CommandText)[1]','nvarchar(max)'))
...
ENABLE TRIGGER [trg_SGMS_CreateDBTrigger] ON ALL SERVER
GO
